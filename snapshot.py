...
class snapShot:
    ...
    def saveImg(self, path=None):
        ...
        if path:
            ...
            self.file_name = os.path.join(
                ...
                "streetview-" +
                self.pov['lat'].replace(".", "_") + "-" +
                self.pov['lon'].replace(".", "_") + "-" +
                self.pov['heading'].replace(".", "_") + "-" +
                self.pov['pitch'].replace(".", "_") +
                ".jpg"
            )
    ...
    def createGeoPackage(self, path):
        ...
        srs = core.QgsCoordinateReferenceSystem("EPSG:4326")
        writer = core.QgsVectorFileWriter(
            path,
            "UTF-8",
            fields,
            core.QgsWkbTypes.Point,
            srs,
            "GPKG"
        )
        del writer
    ...
    def saveGeoPackage(self):
        ...
        urlimg = "https://maps.googleapis.com/maps/api/streetview?size=600x400&location="+self.pov['lat']+","+self.pov['lon']+"&heading="+self.pov['heading']+"&pitch="+self.pov['pitch']+"&sensor=false"+"&fov="+str(fov)+"&key="+self.parent.APIkey
        self.cb.setText(urlimg)
        sfPath = os.path.join(self.sessionDirectory(), "Streetview_snapshots_log.gpkg")
        if not os.path.isfile(sfPath):
            self.createGeoPackage(sfPath)
        vlayer = core.QgsVectorLayer(sfPath, "Streetview_snapshots_log", "ogr")

        if "type" not in [field.name() for field in vlayer.fields()]:
        ...
        if not testIfLayPresent:
            ...
            qsettings.setValue("/qgis/showTips", True)
        ...
        feat.setAttribute(5, self.pov.get('address', ''))
